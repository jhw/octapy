...
# =============================================================================
# LFO Page Tests (AudioPartTrack)
# =============================================================================

class TestAudioPartTrackLfoPage:
    """AudioPartTrack LFO page parameter tests."""

    def test_lfo_speed1_default(self):
        """Test default lfo_speed1 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_speed1 == 32

    def test_set_lfo_speed1(self):
        """Test setting lfo_speed1."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_speed1 = 64
        assert track.lfo_speed1 == 64

    def test_lfo_speed2_default(self):
        """Test default lfo_speed2 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_speed2 == 32

    def test_set_lfo_speed2(self):
        """Test setting lfo_speed2."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_speed2 = 80
        assert track.lfo_speed2 == 80

    def test_lfo_speed3_default(self):
        """Test default lfo_speed3 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_speed3 == 32

    def test_set_lfo_speed3(self):
        """Test setting lfo_speed3."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_speed3 = 100
        assert track.lfo_speed3 == 100

    def test_lfo_depth1_default(self):
        """Test default lfo_depth1 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_depth1 == 0

    def test_set_lfo_depth1(self):
        """Test setting lfo_depth1."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_depth1 = 64
        assert track.lfo_depth1 == 64

    def test_lfo_depth2_default(self):
        """Test default lfo_depth2 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_depth2 == 0

    def test_set_lfo_depth2(self):
        """Test setting lfo_depth2."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_depth2 = 80
        assert track.lfo_depth2 == 80

    def test_lfo_depth3_default(self):
        """Test default lfo_depth3 value."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        assert track.lfo_depth3 == 0

    def test_set_lfo_depth3(self):
        """Test setting lfo_depth3."""
        project = Project.from_template("TEST")
        track = project.bank(1).part(1).track(1)
        track.lfo_depth3 = 100
        assert track.lfo_depth3 == 100

    def test_lfo_all_tracks(self):
        """Test LFO page for all 8 tracks."""
        project = Project.from_template("TEST")
        part = project.bank(1).part(1)
        for track_num in range(1, 9):
            track = part.track(track_num)
            track.lfo_speed1 = track_num * 10
            track.lfo_depth1 = track_num * 5
    ...
    def test_lfo_roundtrip(self, temp_dir):
        ...
        loaded = Project.from_directory(temp_dir / "TEST")
        loaded_track = loaded.bank(1).part(1).track(1)
        assert loaded_track.lfo_speed1 == 40
        assert loaded_track.lfo_speed2 == 60
        assert loaded_track.lfo_speed3 == 80
        assert loaded_track.lfo_depth1 == 30
        assert loaded_track.lfo_depth2 == 50
        assert loaded_track.lfo_depth3 == 70


# =============================================================================
# LFO Page Tests (MidiPartTrack)
# =============================================================================

class TestMidiPartTrackLfoPage:
    """MidiPartTrack LFO page parameter tests."""

    def test_lfo_speed1_default(self):
        """Test default lfo_speed1 value."""
        project = Project.from_template("TEST")
        midi_track = project.bank(1).part(1).midi_track(1)
        assert midi_track.lfo_speed1 == 32

    def test_set_lfo_speed1(self):
        """Test setting lfo_speed1."""
        project = Project.from_template("TEST")
        midi_track = project.bank(1).part(1).midi_track(1)
        midi_track.lfo_speed1 = 64
        assert midi_track.lfo_speed1 == 64

    def test_lfo_depth1_default(self):
        """Test default lfo_depth1 value."""
        project = Project.from_template("TEST")
        midi_track = project.bank(1).part(1).midi_track(1)
        assert midi_track.lfo_depth1 == 0

    def test_set_lfo_depth1(self):
        """Test setting lfo_depth1."""
        project = Project.from_template("TEST")
        midi_track = project.bank(1).part(1).midi_track(1)
        midi_track.lfo_depth1 = 64
        assert midi_track.lfo_depth1 == 64

    def test_midi_lfo_all_tracks(self):
        """Test LFO page for all 8 MIDI tracks."""
        project = Project.from_template("TEST")
        part = project.bank(1).part(1)
        for track_num in range(1, 9):
            midi_track = part.midi_track(track_num)
            midi_track.lfo_speed1 = track_num * 10
            midi_track.lfo_depth1 = track_num * 5
    ...
    def test_midi_lfo_roundtrip(self, temp_dir):
        ...
        project = Project.from_template("TEST")
        midi_track = project.bank(1).part(1).midi_track(1)
        midi_track.lfo_speed1 = 40
        midi_track.lfo_speed2 = 60
        midi_track.lfo_speed3 = 80
        midi_track.lfo_depth1 = 30
        midi_track.lfo_depth2 = 50
        midi_track.lfo_depth3 = 70
        project.to_directory(temp_dir / "TEST")

        loaded = Project.from_directory(temp_dir / "TEST")
        loaded_midi = loaded.bank(1).part(1).midi_track(1)
        assert loaded_midi.lfo_speed1 == 40
        assert loaded_midi.lfo_speed2 == 60
        assert loaded_midi.lfo_speed3 == 80
        assert loaded_midi.lfo_depth1 == 30
        assert loaded_midi.lfo_depth2 == 50
        assert loaded_midi.lfo_depth3 == 70
